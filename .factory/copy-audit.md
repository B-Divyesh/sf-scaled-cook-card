# Landing copy audit

Reviewed 2026-08-30. Sentence counts exclude button labels and image alt text.

| Landing copy | Words | Result |
| --- | ---: | --- |
| Scale recipe amounts in every step. | 6 | Pass |
| For home cooks who need correct quantities while their hands are busy. | 12 | Pass |
| Open a ready pasta card, or paste a recipe you wrote. | 11 | Pass |
| Import YAML or JSON. | 5 | Pass |
| Works offline after the first visit. | 6 | Pass |
| Kitchen Pass checkout is unavailable right now. | 6 | Pass |
| Recipes stay in this browser. | 5 | Pass |
| Bind each amount to its cooking step. | 8 | Pass |
| Start with a recipe you wrote or can import. | 10 | Pass |
| This card focuses on scaling and cooking that recipe. | 9 | Pass |
| The free card keeps scaling, cooking, and export. | 9 | Pass |

No sentence exceeds 22 words. No banned marketing terms appear.

## Terminology

| Concept | Product term |
| --- | --- |
| A recipe shown in the app | cook card |
| The prepared instructions | cooking step |
| A quantity linked to a step | bound ingredient reference |
| The paid license | Kitchen Pass |
| Isolated sample state | demo |
